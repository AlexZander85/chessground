...
function setPremove(state: HeadlessState, orig: cg.Key, dest: cg.Key, meta: cg.SetPremoveMetadata): void {
  ...
  // In multiple-premove mode the visible board is a speculative preview of our own
  // queued replies. This lets the user immediately move the same piece again from
  // its virtual destination while the opponent is still thinking.
  if (!previewMove(state, orig, dest)) return;
  pm.queue.push(move);
...
export function playPremove(state: HeadlessState): boolean {
  ...
  // Keep the just-played move as the new authoritative base, then re-apply only
  // the remaining tail as a speculative board preview. A future server position
  // update will replace this preview before the next head is validated.
  pm.basePieces = new Map(state.pieces);
  if (!rebuildPremovePreview(state)) {
    clearPremove(state, true);
    return true;
  }
  return true;
